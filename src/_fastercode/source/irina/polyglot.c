...
typedef struct {
    ...
    unsigned char castle_flags;      // bit/flag:  0/Wh-Sh  1/Wh-Lo 2/Bl-Sh 3/Bl-Lo
    unsigned char ep_file;           // NO_EP_FILE for no ep_file
    unsigned char to_move;           // 1 is white to move, 0 is black to move
} board_polyglot;
...
Bitmap *RandomPiece     =Random64;
Bitmap *RandomCastle    =Random64+768;
Bitmap *RandomEnPassant =Random64+772;
Bitmap *RandomTurn      =Random64+780;
...
Bitmap hash_polyglot(board_polyglot *board){
    ...
    for(r=0;r<=7;r++){
        for(f=0;f<=7;f++){
            p=(board->pieces)[f][r];
            if(p!='-'){
                p_enc=strchr(piece_names,p)-piece_names;
...
int pboard_from_fen(board_polyglot *board, char *fen){
    ...
    if(to_move_c=='w'){
        board->to_move=PWHITE;
    }else{
        board->to_move=PBLACK;
    }
    p=0;
    while(1){
...
board_polyglot board_p;


Bitmap hash_from_fen(char *fen)
{
    pboard_from_fen(&board_p, fen);
    return hash_polyglot(&board_p);
}
