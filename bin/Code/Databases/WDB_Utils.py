...
def create_training_positions(procesador, wowner, li_registros_selected, li_registros_total, rutina_datos, name):
    ...
    Util.create_folder(Code.configuration.personal_training_folder)
    nom_fns = Util.opj(Code.configuration.personal_training_folder, menuname + ".fns")

    nom_menu = menuname
    if os.path.isfile(nom_fns):
        n = 1
        nom_fns = Util.opj(Code.configuration.personal_training_folder, menuname + "-%d.fns")
        while os.path.isfile(nom_fns % n):
            n += 1
        nom_fns = nom_fns % n
    ...
    tmp_bp = QTUtil2.BarraProgreso1(wowner, menuname, show_time=True)
    tmp_bp.mostrar()
    ...
    if ok:
        ...
        QTUtil2.message_bold(wowner, _("Created") + ":\n\n" + message)
