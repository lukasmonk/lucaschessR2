...
class AnalysisEval:
    ...
    def lv(self, cp: int, factor: float) -> float:
    ...
    def lv_dif(self, cp, cp_best):
        lv_cp = self.lv(cp, self.conf.x_eval_curve_degree)
        lv_cp_best = self.lv(cp_best, self.conf.x_eval_curve_degree)
        return lv_cp_best - lv_cp
