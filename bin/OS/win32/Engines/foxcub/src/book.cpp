 ...
 * The array PG is taken from the Polyglot source code.
...
uint64 *RandomPiece     =PG;
uint64 *RandomCastle    =PG+768;
uint64 *RandomEnPassant =PG+772;
uint64 *RandomTurn      =PG+780;
...
uint64 GetPolyglotKey(const POS *p)
{
    uint64 key = 0;

    for (int sq = 0; sq < 64; sq++) {
        if      (p->tp_bb[P] & BITSET[sq] ) { if ( p->cl_bb[WC] & BITSET[sq] ) key ^= PG[64*1+8*Rank(sq)+File(sq)]; else key ^= PG[64*0+8*Rank(sq)+File(sq)]; }
        else if (p->tp_bb[N] & BITSET[sq] ) { if ( p->cl_bb[WC] & BITSET[sq] ) key ^= PG[64*3+8*Rank(sq)+File(sq)]; else key ^= PG[64*2+8*Rank(sq)+File(sq)]; }
        else if (p->tp_bb[B] & BITSET[sq] ) { if ( p->cl_bb[WC] & BITSET[sq] ) key ^= PG[64*5+8*Rank(sq)+File(sq)]; else key ^= PG[64*4+8*Rank(sq)+File(sq)]; }
        else if (p->tp_bb[R] & BITSET[sq] ) { if ( p->cl_bb[WC] & BITSET[sq] ) key ^= PG[64*7+8*Rank(sq)+File(sq)]; else key ^= PG[64*6+8*Rank(sq)+File(sq)]; }
        else if (p->tp_bb[Q] & BITSET[sq] ) { if ( p->cl_bb[WC] & BITSET[sq] ) key ^= PG[64*9+8*Rank(sq)+File(sq)]; else key ^= PG[64*8+8*Rank(sq)+File(sq)]; }
        else if (p->tp_bb[K] & BITSET[sq] ) { if ( p->cl_bb[WC] & BITSET[sq] ) key ^= PG[64*11+8*Rank(sq)+File(sq)];else key ^= PG[64*10+8*Rank(sq)+File(sq)]; }
    }
...
bool check_book( const char * fname, POS *p, char * move )
{
    ...
    f_book = fopen(fname, "rb");

    if( f_book )
    {
        key = GetPolyglotKey(p);
